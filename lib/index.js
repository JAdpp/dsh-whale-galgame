...
function apply(ctx, config = {}) {
	...
	function sanitizeStoredCgPrompt(raw) {
		...
		const markerAt = prompt.indexOf("画面元素呼应主人最近的经历与工作：");
		if (markerAt < 0) return prompt.slice(0, 1200);
		const theme = prompt.slice(markerAt + 17);
		...
		return prompt.slice(0, markerAt + 17) + theme.slice(0, 360);
	}
	...
	function syncHeroine() {
		...
		s.characterModelLabel = manualCharacter ? p.characterModel || ROSTER[manualCharacter].name : sel ? String(sel.model) : "";
		...
		if (c.chatLines.length === 0) {
			c.chatLines.push({
				who: "heroine",
				text: ROSTER[next].greet
			});
			if (s.lastCurrent && s.lastCurrent !== next) c.chatLines.push({
				who: "narrator",
	...
	function settle() {
		...
		if (decay > 0) s.characters[s.current].chatLines.push({
			who: "narrator",
	...
	function isExplicitUserEvent(node) {
		...
		const sourceKind = String(node.source && node.source.kind || node.message && node.message.source && node.message.source.kind || node.data && node.data.source && node.data.source.kind || "").toLowerCase();
	...
	function cleanThemeText(raw) {
		...
		return raw.replace(/\x60{3}[\s\S]*?\x60{3}/g, " ").replace(/\x60[^\x60\r\n]*\x60/g, " ").split(/\r?\n/).filter((line) => !internal.test(line)).join(" ").replace(/https?:\/\/\S+/gi, " ").replace(/\b[A-Za-z]:[\\/][^\s，。；！？,;]+/g, " ").replace(/(?:^|\s)\/(?:[\w.-]+\/)+[\w.-]+/g, " ").replace(/\b(?:sk-[A-Za-z0-9_-]{8,}|Bearer\s+\S+)\b/gi, " ").replace(/[#>*_~\[\]{}|]+/g, " ").replace(/\s+/g, " ").trim();
	}
	function gatherRecentTheme() {
		try {
			...
			const matches = list.filter((x) => x && x.header && x.header.cwd === root);
	...
	function systemPrompt(ch, c) {
		...
		return ch.system + "\n当前等级：Lv." + c.level + "。亲昵度：" + intimacyFor(c.level) + "\n好感度：" + c.affection + "/" + affectionCap(c.level) + "（满了会升级，你会越来越亲近主人）" + work;
	}
	...
	async function generateChoices(ch, c, lastUser, lastHeroine) {
	...
	function hydrateCharacter(src, legacyVersion) {
	...
	async function handleAction(action, args) {
		...
		switch (action) {
			...
			case "chat": {
					...
					reply = await streamText({
						...
						messages: (() => {
							const msgs = [{
								...
								content: [{
									type: "text",
									text: "（场景：深海女仆工坊的会客厅，暖黄的灯光。" + ch.name + "正在和来访的主人聊天。你只扮演" + ch.name + "这一个角色，不要提到其他角色。）"
								}],
								source: { kind: "user" }
							}];
				...
				if (!checkLevelUp(s.current, c)) c.choices = await generateChoices(ch, c, text, reply);
				await save();
				return view();
			}
			case "sprite-data": {
