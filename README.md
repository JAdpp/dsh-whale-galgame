...
六位角色的自定义设定分别保存。“保存设定”或“恢复默认”只会修改当前角色的上述六项，不会重置其好感度与等级、长期记忆或自定义立绘。若该角色尚未开始真实对话，修改或恢复“首次问候”会原位更新当前开场问候，自动生成的登场旁白保持不变；一旦已有用户或角色对话，就不会再插入、替换或重播历史。CG 外观描述用于之后生成的升级 CG，不会改写图鉴中已经保存的图片。
